        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    // Hide loading state
                    $spinner.hide();
                    $button.prop('disabled', false).text('Load More House Tours');

                    // Parse the returned HTML
                    var $newContent = $(response.data);

                    // Find the new project cards (exclude the load more container)
                    var $newCards = $newContent.filter('.single-house-tour-card');
                    ...
                    $(document).trigger('tours:loaded', [$newCards]);
